...
在一般情况下，除了data[0]和data[1]用于存放USB事件的请求参数外，data[2]至data[7]都可以传递用户参数。
这样在数据量非常小的时候，通过usbFunctionSetup就可以完成全部参数传递了，不需要使用函数
usbFnctionRead和 usbFunctionWrite。这时可以在usbconfig.h中将宏USB_CFG_IMPLEMENT_FN_READ和
USB_CFG_IMPLEMENT_FN_WRITE设置为0，禁止使用函数usbFnctionRead和usbFunctionWrite，
这样可以节省出不少代码空间。 在传递的数据比较多时，就需要使用usbFunctionWrite和usbFnctionRead函数了。
这两个函数具有相同的参数(uchar *data, uchar len)，len表示参数的数量，*data指向数据缓冲区。在编程时，
一般是先在usbFunctionSetup函数中根据计算机发送过来的请求类型和参数来设置一个全局标志，
然后在usbFunctionWrite和usbFnctionRead函数中根据相应全局标志进行不同的功能处理。
*/
